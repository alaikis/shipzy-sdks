...
public class FinanceClient {
    ...
    public FinanceClient(ZymeupClient client) {
        this.client = client;
    }

    public Map<String, Object> listInvoices(int page, int pageSize) throws Exception {
        HttpRequest request = client.newRequest("/api/v1/invoices?page=" + page + "&page_size=" + pageSize).GET().build();
        HttpResponse<String> response = client.send(request);
        return parseResponse(response.body());
    }

    public Map<String, Object> getInvoice(String id) throws Exception {
        HttpRequest request = client.newRequest("/api/v1/invoices/" + id).GET().build();
        HttpResponse<String> response = client.send(request);
        return parseResponse(response.body());
    }
